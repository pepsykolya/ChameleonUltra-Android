//-----------------------------------------------------------------------------
// Crypto1 implementation for mfkey32v2 Android port
// Based on Proxmark3 crypto01.c - stripped for Android NDK
//-----------------------------------------------------------------------------
#include "crypto01.h"
...
struct Crypto1State *lfsr_common_prefix(uint32_t pfx, uint32_t rr, uint8_t ks[8], uint8_t par[8][8], uint32_t no_par) {
    ...
    s = statelist = calloc(1, (sizeof * statelist) << 24);
    if (!s || !odd || !even) {
        free(statelist);
        statelist = 0;
        goto out;
...
void crypto1_init(struct Crypto1State *state, uint64_t key) {
    for (int i = 0; i < 48; i++) {
        state->odd ^= ((key >> (i * 2 + 1)) & 1) << (i + 8);
        state->even ^= ((key >> (i * 2)) & 1) << (i + 8);
    }
}
...
struct Crypto1State *crypto1_create(uint64_t key) {
    struct Crypto1State *state = calloc(1, sizeof(struct Crypto1State));
    if (state) crypto1_init(state, key);
    return state;
}

void crypto1_destroy(struct Crypto1State *state) {
    if (state) {
        crypto1_deinit(state);
        free(state);
    }
}

void crypto1_get_lfsr(struct Crypto1State *state, uint64_t *key) {
    *key = 0;
    for (int i = 23; i >= 0; i--) {
        *key = (*key << 1) | (state->odd >> (i + 8) & 1);
        *key = (*key << 1) | (state->even >> (i + 8) & 1);
    }
}

uint8_t crypto1_bit(struct Crypto1State *s, uint8_t in, int fb) {
    uint32_t feedin = s->odd & 1;
    feedin ^= !!fb & !!(s->odd & 0x000100);
    feedin ^= !!in;
    feedin ^= LF_POLY_EVEN & (s->even >>= 1);
    feedin ^= LF_POLY_ODD & s->odd;
    s->even |= (evenparity32(feedin)) << 23;
    uint32_t t = s->odd, s_odd = s->odd;
    s->odd = s->even;
    s->even = t;
    return filter(s_odd);
}

uint8_t crypto1_byte(struct Crypto1State *s, uint8_t in, int fb) {
    uint8_t ret = 0;
    for (int i = 7; i >= 0; i--) {
        ret |= crypto1_bit(s, BIT(in, i), fb) << i;
    }
    return ret;
}

uint32_t crypto1_word(struct Crypto1State *s, uint32_t in, int fb) {
    uint32_t ret = 0;
    for (int i = 31; i >= 0; i--) {
        ret |= crypto1_bit(s, BEBIT(in, i), fb) << (24 ^ i);
    }
    return ret;
}

uint32_t prng_successor(uint32_t x, uint32_t n) {
    while (n--) x = x >> 1 | (x ^ x >> 2 ^ x >> 3 ^ x >> 5) << 15;
    return x;
}
